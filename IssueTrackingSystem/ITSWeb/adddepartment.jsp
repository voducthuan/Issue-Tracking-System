...
<%@page import="tool.*"%>
<%
    String strSubmit, sDepName, strCopy;
    String strError="",strAlert="";
    int intResult=-1;
    sDepName = Utility.ConvertNullToEmpty(request.getParameter("txtDepName"));
    strSubmit = Utility.ConvertNullToEmpty(request.getParameter("txtAction"));
    if(strSubmit!=""){
        Department objDep = new Department();
        objDep.setDescription(sDepName);
        intResult = objDep.add();
        if(intResult==SysConstant.OK){
              	strAlert = "alert('Create new Despartment sucessfully');";
...
{
  	if(frmAddTeam.txtDepName.value=="")
	{
		alert("Team Name cannot be null");
		frmAddTeam.txtDepName.focus();
		return true;
	}
		return false;
}
			...
			<tr>
				...
				<table border="0" width="40%" id="table7" class="TableBorderFrm">
					...
					<tr>
						...
						<form name="frmAddTeam" method="POST" action="adddepartment.jsp">
							<table border="0" width="100%" id="table8">
								<tr>
									<td class="Label">Department Name</td>
									<td>
									<input type="text" name="txtDepName" size="35" class="TextBoxStyle" maxlength="50"></td>
								</tr>
								<tr>
									<td>&nbsp;</td>
									<td>
