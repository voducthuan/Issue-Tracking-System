...
if(intSubmit>0){
	...
	switch(intSubmit){
        case 1://Insert a new application admin
		...
		if(strSelectedUserID!=""){
	        	objUser.setUserID(Integer.parseInt(strSelectedUserID));
	                Project objProject = new Project();
                        ...
                        objAdminRight.setSystemRightID(2); // 2 = application admin
                ...
                break;
        case 2://Remove selected user of list of application admin
 	   	strSelectedUserID = Utility.ConvertNullToEmpty(request.getParameter("txtSelectedUserID"));
		if(strSelectedUserID!=""){
	        	objUser.setUserID(Integer.parseInt(strSelectedUserID));
	                AdminSystemRight objAdminRight = new AdminSystemRight();
                        objAdminRight.setSystemRightID(2);// 2 = application admin
                        intResult = objUser.removeApplicationAdminRight(objAdminRight);
	                if(intResult==SysConstant.OK){
	                	strAlert = "alert('The selected user has been removed of list of application admin');";
				break;
	                }
	                else
...
setTemp = objUser.getListUserNotApplicationAdmin();
...
setTemp = objUser.getListUserApplicationAdmin();
iter = setTemp.iterator();
for (int i=0; iter.hasNext();i++ ) {
	objUser = (User) iter.next();
        if((i%2)==0){
...
function Insert(){
      document.frmAppAdmin.txtAction.value=1;
      document.frmAppAdmin.submit();
}

function Delete(p_intUserID){
      document.frmAppAdmin.txtAction.value=2;
      document.frmAppAdmin.txtSelectedUserID.value=p_intUserID;
      document.frmAppAdmin.submit();
}
</script>
<link href="style/formstyle.css" type=text/css rel=stylesheet>
<title>Manage application administrators</title>
</head>
...
<jsp:include page="menuapplicationadmin.jsp" flush="true"/>
	...
	<tr>
		...
		<table border="0" width="100%" id="table6" bgcolor="#FFFFFF">
			...
			<tr>
				<td align="center">
				<form name = "frmAppAdmin" method="POST" action="manageappadmin.jsp">
